...
function EditEmployee(props) {
    ...
    const editOldEmployee = async () => {
        try {
            const response = await axios.put('http://localhost:3036/employees/' + props.employeeId, employeeInfo);
            if (response) {
                props.setEmployeeEdited();
            }
        } catch (e) {
            console.log(e);
        }
    ...
    return (
        <div className="main-view-addbtn">
            ...
            <button className="btn btn-secondary" onClick={() => editOldEmployee()}>Update Employee</button>
        </div>
    );
}
